...
@Transactional
public class GenericAttachmentsExecutor extends FileExecutor {
	...
	String execute(HttpServletRequest request, List<FileItem> sessionFiles)
		...
		String err = null;
		String transactionRefId = request.getParameter("transactionRefId");
			...
			if (false == item.isFormField()) {
				try {
					...
					String fileName = getFilePath() + File.separator
							+ IDUtils.generateId() + "-GENERIC-" + name;
					attachment.setFileName(fileName);

					logger.info("Generic attachment >>>>" + fileName + " "
							+ fieldName);

					if (transactionRefId != null) {
						Transaction trx = dao.findByRefId(transactionRefId,
								Transaction.class);
						attachment.setTransaction(trx);
					}
