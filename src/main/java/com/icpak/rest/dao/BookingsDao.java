...
public class BookingsDao extends BaseDao {
	...
	public Booking getBySponsorEmail(String email, Long eventId) {
		Booking booking = getSingleResultOrNull(getEntityManager()
				.createQuery("from Booking u where u.contact.email=:email and event.id=:eventId and isActive=1")
				.setParameter("email", email).setParameter("eventId", eventId));
		if (booking == null) {
			logger.debug("Booking for sponsor email:" + email + " not found for event:" + eventId);
		} else {
			logger.debug("Duplicate Booking found for email:" + email + " and eventRefId:" + eventId);
		}
		return booking;
	}
	...
	public void importAllEvents() {
		String sqlQuery = "select oldSystemId,refId from icpakdb.event " + "where oldSystemId IN (480,479,474,469)";
		// String sqlQuery =
	...
	public List<Booking> getAllBookings(Long eventId) {
		return getResultList(getEntityManager().createQuery("from Booking b where b.event.id=:Id order by created")
	...
	public List<MemberBookingDto> getMemberBookings(String memberRefId, int offset, int limit) {
		String sql = "select b.refId bookingRefId, " + "e.refId eventRefId, " + "d.refId delegateRefId, " + "e.name, "
				...
				+ "b.paymentStatus,b.isActive, " + "a.hotel " + "from event e "
				...
				+ "left join invoice i on (i.bookingRefId=b.refId) " + "where d.memberRefId=:memberRefId";
		...
		for (Object[] row : rows) {
			...
			Integer bookingStatus = (value = row[i++]) == null ? null : ((Number) value).intValue();
			...
			dto.setPaymentStatus(
					PaymentStatus.PAID.ordinal() == paymentStatus ? PaymentStatus.PAID : PaymentStatus.NOTPAID);
	...
	public int getGenericDelegateCount(Long bookingId, String bookingType, boolean isMember) {
		Number number = null;
		String sql = "select count(*) " + "from delegate d where booking_id=:bId";
		...
		Query query = getEntityManager().createNativeQuery(sql).setParameter("bId", bookingId);
		/* Parameters */
		if (bookingType != null && !bookingType.isEmpty()) {
			if (bookingType.equals("paid")) {
	...
	public int getStaffTransactionCount(Long eventId, String offlinePaymentType) {
		...
		if (offlinePaymentType != null && !offlinePaymentType.isEmpty()) {
			if (offlinePaymentType.equals("receipt")) {
				sql = sql.concat(" and receiptNo is not null and receiptNo<>''");
			}
	...
	public BookingSummaryDto getBookingSummary(String eventId) {
		...
		int i = 0;
		Object value = null;
		Integer totalDelegates = (value = row[i++]) == null ? null : ((BigDecimal) value).intValue();
		Integer totalPaid = (value = row[i++]) == null ? null : ((BigDecimal) value).intValue();
		Integer totalWithAccomodation = (value = row[i++]) == null ? null : ((BigDecimal) value).intValue();
		Integer totalAttended = (value = row[i++]) == null ? null : ((BigDecimal) value).intValue();
		Integer totalCancelled = (value = row[i++]) == null ? null : ((BigDecimal) value).intValue();
		Integer totalCancelledMembers = (value = row[i++]) == null ? null : ((BigDecimal) value).intValue();
		Integer totalCancelledNonMembers = (value = row[i++]) == null ? null : ((BigDecimal) value).intValue();
		Integer totalMembers = (value = row[i++]) == null ? null : ((BigDecimal) value).intValue();
		Integer totalNonMembers = (value = row[i++]) == null ? null : ((BigDecimal) value).intValue();
		Integer totalPaidMembers = (value = row[i++]) == null ? null : ((BigDecimal) value).intValue();
		Integer totalPaidNonMembers = (value = row[i++]) == null ? null : ((BigDecimal) value).intValue();

		System.err.println(totalDelegates + " " + totalPaid + " " + totalWithAccomodation + " " + totalAttended + " "
				+ totalCancelled);
	...
	private BookingSummaryDto getEventSummary(BookingSummaryDto summary, String eventId) {
		String sql = "select totalMpesaPayments,totalCardsPayment,totalOfflinePayment,"
				+ "totalCredit,totalReceiptPayment from event " + "where refId=:eventId";
		Query query = getEntityManager().createNativeQuery(sql).setParameter("eventId", eventId);
		Object[] row = getSingleResultOrNull(query);
		int i = 0;
		Object value = null;
		Integer totalMpesaPayments = (value = row[i++]) == null ? null : (Integer) value;
		Integer totalCardsPayment = (value = row[i++]) == null ? null : ((Integer) value);
		Integer totalOfflinePayment = (value = row[i++]) == null ? null : ((Integer) value);
		Integer totalCredit = (value = row[i++]) == null ? null : ((Integer) value);
		Integer totalReceiptPayment = (value = row[i++]) == null ? null : ((Integer) value);
	...
	public int getDelegateCount(String eventId, String searchTerm, String accomodationRefId, String bookingStatus) {
		Number number = null;
		String sql = "select count(*) " + "from delegate d inner join booking b on (d.booking_id=b.id) "
		...
		if (searchTerm != null && !searchTerm.isEmpty()) {
			sql = sql.concat(" and " + "(d.memberRegistrationNo like :searchTerm or " + "d.email like :searchTerm or "
					+ "d.fullName like :searchTerm or " + "d.ern like :searchTerm or "
					+ "b.`E-mail` like :searchTerm or " + "b.company like :searchTerm or "
					+ "b.Contact like :searchTerm or " + "i.documentNo like :searchTerm)");
		}
	...
	public List<DelegateDto> getAllDelegates(String passedRefId, Integer offset, Integer limit, String searchTerm,
		...
		return getAllDelegates(passedRefId, offset, limit, searchTerm, false, accomodationRefId, bookingStatus);
	}
	...
	public List<String> correctDoubleBookings(String eventRefId) {
		String sql = "SELECT d.memberRegistrationNo,COUNT(memberRegistrationNo) FROM delegate d "
				...
				+ " (SELECT id FROM event WHERE refId = :eventRefId)" + " and b.`E-Mail`='anne.njagi@icpak.com'"
		...
		for (Object o[] : rows) {
			int i = 0;
			Object value = null;
			String memberRegistrationNo = (value = o[i++]) == null ? null : value.toString();
			Integer count = (value = o[i++]) == null ? null : ((BigInteger) value).intValue();
			System.err.println(">>memberNo::" + memberRegistrationNo + ">>count::" + count);
	...
	public List<DelegateDto> getAllDelegates(String passedRefId, Integer offset, Integer limit, String searchTerm,
		...
		return getAllDelegates(passedRefId, offset, limit, searchTerm, isByQrCode, accomodationRefId, bookingStatus,
				null);
	}

	public List<DelegateDto> getAllDelegates(String passedRefId, Integer offset, Integer limit, String searchTerm,
			boolean isByQrCode, String accomodationRefId, String bookingStatus, String memberRegistrationNo) {
		...
		String sql = "select b.refId as bookingRefId,b.bookingDate," + "b.company,b.Contact, b.`E-Mail`,b.`Phone No_`,"
		...
		if (memberRegistrationNo != null && !memberRegistrationNo.isEmpty()) {
			sql = sql.concat(" and d.memberRegistrationNo=:memberRegistrationNo");
		}
		...
		if (searchTerm != null && !searchTerm.isEmpty()) {
			if (isByQrCode) {
				...
				sql = sql.concat(
						" and " + "(d.memberRegistrationNo like :searchTerm or " + "d.email like :searchTerm or "
								+ "d.fullName like :searchTerm or " + "d.ern like :searchTerm or "
								+ "b.`E-mail` like :searchTerm or " + "b.company like :searchTerm or "
								+ "b.Contact like :searchTerm or " + "i.documentNo like :searchTerm)");
			}
		...
		for (Object o[] : rows) {
			...
			String contactPhone = (value = o[i++]) == null ? null : value.toString();
			...
			String fullName = (value = o[i++]) == null ? null : value.toString();
			String phoneNumber = (value = o[i++]) == null ? null : value.toString();
			...
			Integer isBookingActive = (value = o[i++]) == null ? null : (Integer) value;
			Integer isDelegateActive = (value = o[i++]) == null ? null : (Integer) value;
			String invoiceNo = (value = o[i++]) == null ? null : value.toString();
			...
			if (delegatePaymentStatus != null) {
				delegateDto.setDelegatePaymentStatus(PaymentStatus.valueOf(delegatePaymentStatus));
			}
			if (attendance == 0) {
	...
	public void deleteExistingDelegates(String bookingId) {
		logger.error("Deleting existing delegates for booking id ======><><<>>>>>>>>===" + bookingId);
		Query deleteQuery = getEntityManager().createNativeQuery("delete from delegate " + "where booking_id = ( "
				+ "select id from booking " + "where refId = :bookingRefId)");
