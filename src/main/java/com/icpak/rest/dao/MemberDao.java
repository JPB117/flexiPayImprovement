...
public class MemberDao extends BaseDao {
	...
	public Integer getMembersCount(String searchTerm, String citySearchTerm, String categoryName) {
		StringBuffer sql = new StringBuffer("select count(*) " + " from allicpakmembers_5 m");
	...
	public List<MemberDto> searchMembersFromOldTable(String searchTerm, String citySearchTerm, String categoryName,
		...
		StringBuffer sql = new StringBuffer(
				"select m.No_,m.Name,m.Address,m.City," + "m.`Customer Posting Group`,m.`Customer Type`,"
						+ "m.Status,m.PractisingNo " + " from allicpakmembers_5 m ");
		Map<String, Object> params = appendParameters(searchTerm, citySearchTerm, categoryName, sql);
		Query query = getEntityManager().createNativeQuery(sql.toString());
		...
		for (Object[] row : rows) {
			...
			Double memberNo = (value = row[i++]) == null ? null : (Double) value;
			String fullName = (value = row[i++]) == null ? null : value.toString();
			String address = (value = row[i++]) == null ? null : value.toString();
			String city = (value = row[i++]) == null ? null : value.toString();
			String customerPostingGroup = (value = row[i++]) == null ? null : value.toString();
			String customerType = (value = row[i++]) == null ? null : value.toString();
			Double status = (value = row[i++]) == null ? null : ((Double) value);
			String practisingNo = (value = row[i++]) == null ? null : value.toString();
			
			MemberDto memberDto = new MemberDto();
			
			memberDto.setMemberNo(memberNo == null ? "Not Available" : Double.toString(memberNo));
			memberDto.setFullName(fullName);
