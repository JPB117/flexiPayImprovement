	...
	public List<CPDDto> getAllCPDs(String passedMemberRefId, Integer offSet, Integer limit, Date startDate,
		...
		StringBuffer sql = new StringBuffer(
				"select c.id,c.created,c.refId as cpdRefId," + "c.title,c.status,c.memberRegistrationNo,c.memberRefId, "
						+ "u.fullName,c.startDate,c.endDate,c.cpdHours,c.organizer,c.category " + "from cpd c "
		...
		for (Object[] row : rows) {
			...
			String category = (value = row[i++]) == null ? null : value.toString();
			...
			cpd.setCategory(category == null ? null : CPDCategory.valueOf(category));
			cpds.add(cpd);
		}
		return cpds;
	}
