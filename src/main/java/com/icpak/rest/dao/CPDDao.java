...
public class CPDDao extends BaseDao {
	...
	public List<MemberCPDDto> getMemberCPD(String searchTerm, Integer offset, Integer limit) {
		...
		Map<String, Object> params = appendParameters(sql, null, null, null, searchTerm);
			...
			if (searchTerm != null) {
				if (!searchTerm.equals("")) {
					sql.append(" and (concat(u.firstName,' ',u.lastName) like :searchTerm or"
							+ " c.title like :searchTerm or " + " c.memberRegistrationNo like :searchTerm)");
	...
	public List<Attachment> getAllCPDAttachment() {
		String sql = "From Attachment WHERE cpdid is not null";
		Query query = getEntityManager().createQuery(sql);

		return getResultList(query,0,30);
	}
	...
	public void dumpBlobToFile() {
		...
		for (Attachment attachment : getAllCPDAttachment()) {
			if (attachment.getAttachment() != null) {
				...
				String query = "UPDATE attachment SET " + "fileName=:fileName,attachment=:attachment " + "WHERE id=:id";
