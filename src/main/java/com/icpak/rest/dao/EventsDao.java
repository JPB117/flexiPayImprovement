...
public class EventsDao extends BaseDao {
	...
	public List<Event> getAllEvents(Integer offSet, Integer limit, EventType type, String searchTerm) {
		...
		if (searchTerm.isEmpty()) {
			...
			if (type == null) {
				...
				return getResultList(
						getEntityManager().createQuery("from Event where isActive=1 order by startDate DESC"), offSet,
						limit);
		...
		return getResultList(getEntityManager().createQuery(query).setParameter("searchTerm", "%" + searchTerm + "%"),
				offSet, limit);
