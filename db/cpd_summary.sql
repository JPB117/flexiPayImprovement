...
UPDATE 
navmember JOIN
(SELECT memberRegistrationNo, SUM(cpdHours) AS YEAR_2011 from cpd WHERE YEAR(startdate) = 2011 and memberRegistrationNo=p_memberRegistrationNo and status='Approved') AS cpd_2011 ON cpd_2011.memberRegistrationNo = navmember.No_ 
JOIN (SELECT memberRegistrationNo, SUM(cpdHours) AS YEAR_2012 from cpd WHERE YEAR(startdate) = 2012 and memberRegistrationNo=p_memberRegistrationNo and status='Approved') AS cpd_2012 ON cpd_2012.memberRegistrationNo = navmember.No_ 
JOIN (SELECT memberRegistrationNo, SUM(cpdHours) AS YEAR_2013 from cpd WHERE YEAR(startdate) = 2013 and memberRegistrationNo=p_memberRegistrationNo and status='Approved') AS cpd_2013 ON cpd_2013.memberRegistrationNo = navmember.No_ 
JOIN (SELECT memberRegistrationNo, SUM(cpdHours) AS YEAR_2014 from cpd WHERE YEAR(startdate) = 2014 and memberRegistrationNo=p_memberRegistrationNo and status='Approved') AS cpd_2014 ON cpd_2014.memberRegistrationNo = navmember.No_ 
JOIN (SELECT memberRegistrationNo, SUM(cpdHours) AS YEAR_2015 from cpd WHERE YEAR(startdate) = 2015 and memberRegistrationNo=p_memberRegistrationNo and status='Approved') AS cpd_2015 on cpd_2015.memberRegistrationNo = navmember.No_
JOIN (SELECT memberRegistrationNo, SUM(cpdHours) AS YEAR_2016 from cpd WHERE YEAR(startdate) = 2016 and memberRegistrationNo=p_memberRegistrationNo and status='Approved') AS cpd_2016 on cpd_2016.memberRegistrationNo = navmember.No_
